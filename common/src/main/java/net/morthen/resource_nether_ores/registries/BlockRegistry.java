package net.morthen.resource_nether_ores.registries;
...
public class BlockRegistry {
    ...
    public static final RegistryObject<Block> NETHER_LAPIS_ORE = register("nether_lapis_ore", properties -> new DropExperienceBlock(UniformInt.of(1, 5), properties), BlockBehaviour.Properties.ofFullCopy(Blocks.LAPIS_ORE));
    ...
    private static RegistryObject<Block> register(String id, Function<BlockBehaviour.Properties, Block> function, BlockBehaviour.Properties properties) {
        RegistryObject<Block> toReturn = BLOCKS.register(id, () -> function.apply(properties.setId(blockKey(id))));
        registerItem(id, props -> new BlockItem(toReturn.get(), props.useBlockDescriptionPrefix()));

        return toReturn;
    }

    private static void registerItem(String id, Function<Item.Properties, Item> function) {
        ITEMS.register(id, () -> function.apply(new Item.Properties().setId(itemKey(id))));
    }
